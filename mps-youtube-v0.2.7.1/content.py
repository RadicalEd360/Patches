...
from . import __version__, g, config, c
...
def generate_songlist_display(song=False, zeromsg=None):
	...
	#title bar
    hrow = c.sh + fmt % titles + c.w
    out = "\n" + hrow + "\n"

    for n, x in enumerate(g.model[:max_results]):
        col = (c.s1 if n % 2 == 0 else c.s2) if not song else c.b
...
def generate_playlist_display():
    ...
    head = (c.ph, "Item", "Playlist", "Author", "Updated", "Count", c.w)
    out = "\n" + fmthd % head

    for n, x in enumerate(g.ytpls):
        col = (c.p1 if n % 2 == 0 else c.p2)
...
def logo(col=None, version=__version__):
    """ Return text logo. """
    #col = col if col else random.choice((c.g, c.r, c.y, c.b, c.p, c.w))
    col = random.choice((c.g, c.r, c.y, c.b, c.p, c.w))
    logo_txt = random.choice((r"""
...
▝▀█▀▘ █▄█  ▐▌     █    █ ▝▀█▀▘     ▐▌   ▐▀▀█▖
  ...
  █   █ █  ▐▙▄  ▟▘   ▟▘    █   ▐█▌ ▐█▄█▘▐▄▄█▘
  ▀  ▝▀ ▀▘  ▀▀ ▗▛   ▗▛     ▀    ▀  ▝▘▀▘  ▀▀▘ 
        Highly Versatile Console Tuber
   ----------------------------------------
                 """))

    version = " v" + version if version else "asdf"
